...
def _mock_clients(mock_host, pbs_snaps, pve_meta, restic_by_vm, untagged=None):
    """Patch all three backend clients and AgentClient for /api/host/<id>/items and jobs."""
    _app._cache.clear()

    from mqtt_manager import MQTT_CACHE, MQTT_CACHE_LOCK
    base = f"proxmox/{HOST_ID}"

    # Determine if restic is busy (e.g. from the res_mock mock in tests)
    restic_busy = False
    ...
    with MQTT_CACHE_LOCK:
        MQTT_CACHE.clear()
        MQTT_CACHE[f"{base}/state/ready"] = {"pbs_ok": True}
        MQTT_CACHE[f"{base}/state/restic_busy"] = restic_busy
        MQTT_CACHE[f"{base}/storage"] = storage_info
        MQTT_CACHE[f"{base}/vms/index"] = all_vmids

        for vmid in all_vmids:
            vmid_int = int(vmid) if str(vmid).isdigit() else vmid
            meta = pve_meta.get(vmid_int, pve_meta.get(str(vmid), {}))

            pbs_btype = "vm"
            for g in pbs_snaps:
                if g["pve_id"] == vmid_int:
                    pbs_btype = g.get("backup_type", "vm")
            orphan_type = "lxc" if pbs_btype == "ct" else "vm"

            MQTT_CACHE[f"{base}/vm/{vmid}/meta"] = {
                "vmid": vmid,
                "name": meta.get("name", f"id-{vmid}"),
                "type": meta.get("type", orphan_type),
                "os": meta.get("os", "linux"),
                "template": meta.get("template", False),
                "in_pve": vmid_int in pve_meta or str(vmid) in pve_meta,
            }
            ...
            vm_restic = sorted(
                restic_by_vm.get(vmid_int, restic_by_vm.get(str(vmid), [])),
            ...
            for snap in raw_snaps:
                ...
                if rs is None:
                    old_entries_sorted = sorted(
                        [r for r in vm_restic if "pbs_time" not in r] + (untagged or []),
                        key=lambda r: r["ts"], reverse=True
                    )
                    rs = next((r for r in old_entries_sorted if r["ts"] > bt), None)
            ...
            MQTT_CACHE[f"{base}/vm/{vmid}/pbs"] = {"snapshots": annotated}
            MQTT_CACHE[f"{base}/vm/{vmid}/restic"] = {"snapshots": vm_restic}
            MQTT_CACHE[f"{base}/vm/{vmid}/summary"] = {
                "snapshot_count": len(annotated),
                "age_hours": 1.0 if annotated else None,
                "has_local": any(s.get("local") for s in annotated),
                "has_cloud": any(s.get("cloud") for s in annotated),
                "status": "ok",
            }
    ...
    with patch.dict(_app.HOSTS, {HOST_ID: mock_host}, clear=True), \
        ...
        # Populate AgentClient mock triggers for pure MQTT jobs
        agent_mock.backup.return_value = "op123"
        agent_mock.restore.return_value = "op123"
        agent_mock.delete.return_value = "op123"
        agent_mock.delete_all.return_value = "op123"
        agent_mock.backup_restic.return_value = "op123"
        agent_mock.delete_restic.return_value = "op123"
        agent_mock.restore_datastore.return_value = "op123"
        agent_mock.wait_for_op.return_value = True

        from agent_client import AgentClient
        agent_real = AgentClient(mock_host)
        agent_mock.get_items.side_effect = agent_real.get_items
        agent_mock.get_schedules.side_effect = agent_real.get_schedules
        agent_mock.get_settings.side_effect = agent_real.get_settings
...
class TestResticSnapshotCoverage:
    ...
    def _get_restic_snaps(self, flask_client, mock_host, flat_snaps, pbs_groups):
        ...
        # Group flat_snaps by VMID based on covers
        restic_by_vm = {}
        for snap in flat_snaps:
            covers = snap.get("covers", [])
            if not covers:
                restic_by_vm.setdefault(301, []).append(snap)
            for cov in covers:
                vmid = cov.get("vmid")
                if vmid is not None:
                    restic_by_vm.setdefault(int(vmid), []).append({
                        "id": snap.get("id"),
        ...
        with MQTT_CACHE_LOCK:
            MQTT_CACHE.clear()
            MQTT_CACHE[f"{base}/state/restic_busy"] = False
            for vmid, snaps in restic_by_vm.items():
                MQTT_CACHE[f"{base}/vm/{vmid}/restic"] = {"snapshots": snaps}
            
            # Format and cache PBS snapshots
            for g in pbs_groups:
                vmid = g["pve_id"]
                snaps_formatted = []
                for snap in g.get("snapshots", []):
                    snaps_formatted.append({
                        **snap,
                        "local": True,
                        "cloud": False,
                    })
                MQTT_CACHE[f"{base}/vm/{vmid}/pbs"] = {"snapshots": snaps_formatted}
...
class TestSchedulesEndpoint:
    ...
    def _get(self, flask_client, mock_host,
        ...
        from mqtt_manager import MQTT_CACHE, MQTT_CACHE_LOCK
        base = f"proxmox/{HOST_ID}"
        filtered_pbs_retention = []
        if pbs_prune_jobs:
            filtered_pbs_retention = [j for j in pbs_prune_jobs if j.get("store") == mock_host.pbs_datastore]

        with MQTT_CACHE_LOCK:
            MQTT_CACHE[f"{base}/schedules"] = {
                "pbs_jobs": pbs_jobs or [],
                "pbs_running": False,
                "restic_next": restic_next,
                "restic_running": restic_running,
                "restic_retention": restic_retention or {},
                "pbs_retention": filtered_pbs_retention,
            }
...
class TestHostMgmt:
    ...
    def test_hostconfig_minimal_fields(self):
        """HostConfig can be created with only id + label."""
        from config import HostConfig
        h = HostConfig(id="x", label="X")
        assert h.pve_url == ""
        assert h.pbs_url == ""
    ...
    def test_add_host_duplicate_id_gets_suffix(self, flask_client):
        from config import HostConfig
        existing = HostConfig(id="pi5", label="Pi5 old")
